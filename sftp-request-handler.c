...
static void post_open_to_sink(u_int32_t id)
{
	...
	len = snprintf(buff_sink, SFTP_MAX_HANDLER_MSG_LENGTH,
            "user=%-20s id=%-10d rqst=%-10s path='%.*s'\n",
            pw_sftp->pw_name, id, "open", (int)len, path);
...
static void post_close_to_sink(u_int32_t id)
{
	...
	if (hptr != NULL) {
	   ...
	   len = snprintf(buff_sink, SFTP_MAX_HANDLER_MSG_LENGTH,
               "user=%-20s id=%-10d rqst=%-10s path='%s'\n",
               pw_sftp->pw_name, id, "close", hptr->name);
	   rc = write_to_handler_sink(buff_sink, len);
	   if (rc < 1)
	   {
...
static void post_read_to_sink(u_int32_t id)
{
	...
	if (hptr != NULL) {
	   ...
	   len = snprintf(buff_sink, SFTP_MAX_HANDLER_MSG_LENGTH,
               "user=%-20s id=%-10d rqst=%-10s path='%s' off=%lld max=%d\n",
               pw_sftp->pw_name, id, "read", hptr->name, (long long)off, (int)max);
	   rc = write_to_handler_sink(buff_sink, len);
	   if (rc < 1)
	   {
...
static void post_write_to_sink(u_int32_t id)
{
	...
	if (hptr != NULL) {
	   ...
	   len = snprintf(buff_sink, SFTP_MAX_HANDLER_MSG_LENGTH,
               "user=%-20s id=%-10d rqst=%-10s path='%s' off=%lld cnt=%d\n",
               pw_sftp->pw_name, id, "write", hptr->name, (long long)off, (int)cnt);
	   rc = write_to_handler_sink(buff_sink, len);
	   if (rc < 1)
	   {
...
static void post_stat_to_sink(u_int32_t id)
{
	...
	len = snprintf(buff_sink, SFTP_MAX_HANDLER_MSG_LENGTH,
            "user=%-20s id=%-10d rqst=%-10s path='%.*s'\n",
            pw_sftp->pw_name, id, "stat", (int)len, path);
	rc = write_to_handler_sink(buff_sink, len);
	if (rc < 1)
	{
...
static void post_lstat_to_sink(u_int32_t id)
{
	...
	len = snprintf(buff_sink, SFTP_MAX_HANDLER_MSG_LENGTH,
            "user=%-20s id=%-10d rqst=%-10s path='%.*s'\n",
            pw_sftp->pw_name, id, "lstat", (int)len, path);
	rc = write_to_handler_sink(buff_sink, len);
	if (rc < 1)
	{
...
static void post_fstat_to_sink(u_int32_t id)
{
	...
	if (hptr != NULL) {
	   ...
	   len = snprintf(buff_sink, SFTP_MAX_HANDLER_MSG_LENGTH,
               "user=%-20s id=%-10d rqst=%-10s path='%s'\n",
               pw_sftp->pw_name, id, "fstat", hptr->name);
	   rc = write_to_handler_sink(buff_sink, len);
	   if (rc < 1)
	   {
...
static void post_setstat_to_sink(u_int32_t id)
{
	...
	len = snprintf(buff_sink, SFTP_MAX_HANDLER_MSG_LENGTH,
            "user=%-20s id=%-10d rqst=%-10s path='%.*s'\n",
            pw_sftp->pw_name, id, "setstat", (int)len, path);
	rc = write_to_handler_sink(buff_sink, len);
	if (rc < 1)
	{
...
static void post_fsetstat_to_sink(u_int32_t id)
{
	...
	if (hptr != NULL) {
	   ...
	   len = snprintf(buff_sink, SFTP_MAX_HANDLER_MSG_LENGTH,
               "user=%-20s id=%-10d rqst=%-10s path='%s'\n",
               pw_sftp->pw_name, id, "fsetstat", hptr->name);
	   rc = write_to_handler_sink(buff_sink, len);
	   if (rc < 1)
	   {
...
static void post_opendir_to_sink(u_int32_t id)
{
	...
	len = snprintf(buff_sink, SFTP_MAX_HANDLER_MSG_LENGTH,
            "user=%-20s id=%-10d rqst=%-10s path='%.*s'\n",
            pw_sftp->pw_name, id, "opendir", (int)len, path);
	rc = write_to_handler_sink(buff_sink, len);
	if (rc < 1)
	{
...
static void post_readdir_to_sink(u_int32_t id)
{
	...
	if (hptr != NULL) {
	   ...
	   len = snprintf(buff_sink, SFTP_MAX_HANDLER_MSG_LENGTH,
               "user=%-20s id=%-10d rqst=%-10s path='%s'\n",
               pw_sftp->pw_name, id, "readdir", hptr->name);
	   rc = write_to_handler_sink(buff_sink, len);
	   if (rc < 1)
	   {
...
static void post_remove_to_sink(u_int32_t id)
{
	...
	len = snprintf(buff_sink, SFTP_MAX_HANDLER_MSG_LENGTH,
            "user=%-20s id=%-10d rqst=%-10s path='%.*s'\n",
            pw_sftp->pw_name, id, "remove", (int)len, path);
	rc = write_to_handler_sink(buff_sink, len);
	if (rc < 1)
	{
...
static void post_mkdir_to_sink(u_int32_t id)
{
	...
	len = snprintf(buff_sink, SFTP_MAX_HANDLER_MSG_LENGTH,
            "user=%-20s id=%-10d rqst=%-10s path='%.*s'\n",
            pw_sftp->pw_name, id, "mkdir", (int)len, path);
	rc = write_to_handler_sink(buff_sink, len);
	if (rc < 1)
	{
	   error("Encountered error during write to SFTP handler sink in post_opendir_to_sink: %d", errno);
...
static void post_rmdir_to_sink(u_int32_t id)
{
	...
	len = snprintf(buff_sink, SFTP_MAX_HANDLER_MSG_LENGTH,
            "user=%-20s id=%-10d rqst=%-10s path='%.*s'\n",
            pw_sftp->pw_name, id, "rmdir", (int)len, path);
	rc = write_to_handler_sink(buff_sink, len);
	if (rc < 1)
	{
...
static void post_realpath_to_sink(u_int32_t id)
{
	...
	len = snprintf(buff_sink, SFTP_MAX_HANDLER_MSG_LENGTH,
            "user=%-20s id=%-10d rqst=%-10s path='%.*s'\n",
            pw_sftp->pw_name, id, "realpath", (int)len, path);
	rc = write_to_handler_sink(buff_sink, len);
	if (rc < 1)
	{
...
static void post_rename_to_sink(u_int32_t id)
{
	...
	len = snprintf(buff_sink, SFTP_MAX_HANDLER_MSG_LENGTH,
            "user=%-20s id=%-10d rqst=%-10s path='%.*s' npath='%.*s'\n",
            pw_sftp->pw_name, id, "rename", (int)len, path, (int)nlen, npath);
	rc = write_to_handler_sink(buff_sink, len);
	if (rc < 1)
	{
	   error("Encountered error during write to SFTP handler sink in post_opendir_to_sink: %d", errno);
...
static void post_readlink_to_sink(u_int32_t id)
{
	...
	len = snprintf(buff_sink, SFTP_MAX_HANDLER_MSG_LENGTH,
            "user=%-20s id=%-10d rqst=%-10s path='%.*s'\n",
            pw_sftp->pw_name, id, "readlink", (int)len, path);
	rc = write_to_handler_sink(buff_sink, len);
	if (rc < 1)
	{
...
static void post_symlink_to_sink(u_int32_t id)
{
	...
	len = snprintf(buff_sink, SFTP_MAX_HANDLER_MSG_LENGTH,
            "user=%-20s id=%-10d rqst=%-10s path='%.*s' newpath='%.*s'\n",
            pw_sftp->pw_name, id, "symlink", (int)len, path, (int)nlen, npath);
	rc = write_to_handler_sink(buff_sink, len);
	if (rc < 1)
	{
	   error("Encountered error during write to SFTP handler sink in post_opendir_to_sink: %d", errno);
